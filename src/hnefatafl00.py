...
# TODO: Combine the HnefataflGameBoard and HnefataflGame classes to remove redundant referencing.
class HnefataflGameBoard(TaflBoard):
    def __init__(self):
...
class HnefataflGame(UniqueID):
    def __init__(self, gameboard: HnefataflGameBoard = None):
        super().__init__()
        self.gameboard = gameboard
        if self.gameboard is None:
            self.gameboard = HnefataflGameBoard()
            self.gameboard.init_board_start()
    ...
    def valid_position_change(
        ...
        for i in range(minimum + 1, maximum):
            loc = old_location
            loc[axis] = i
            if self.gameboard.board[loc[0]][loc[1]] is not None:
                return False
        return True
    ...
    def is_pawn(self, location: Tuple[int, int], color: HnefataflPieceColor) -> bool:
        if self.is_valid_location(location):
            value = self.gameboard.board[location[0]][location[1]]
